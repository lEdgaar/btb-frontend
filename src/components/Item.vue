...
<template>
  <div style="display: flex; justify-content: space-between;">
    ...
    <div style="justify-content: space-between; flex: 2; display: flex;  flex-direction: column;">
      <h4 md="2">{{ item.title }}</h4>
      <h6 md="2">{{ item.name }}</h6>
    </div>

    <div style="flex:3">
      <p md="2">Descripción: {{ item.description }}</p>
      <button  @click="navigateToGroup(item.id)"  class="btn btn-md btn-success" >Entrar</button>
    </div>
  </div>
  <b-row id="item"> </b-row>
</template>