...
const listGroup = async() => {
  ...
  try {
    const response = await axios.get('http://localhost:9010/asset/', {
        headers: {
    ...
    <b-row id="Itemlist">
      ...
      <b-col v-if="items.length > 0" md="12">
        <b-card v-for="item in items" :key="item.value" class="m-2">
          <Asset :item="item" :logged="isAdmin" :isAdmin="isAdmin" :eventEdit="setOperational" :eventDelete="deleteItemModal" />
        </b-card>
      </b-col>
    </b-row>
