<template>
  ...
  <br>
    <h1>{{form.name}}</h1>
    <br>
    Social Trading: {{form.enableSocialTrading}}
    <br>
...
const fetchGroup = async () => {
  ...
  try {
    const response = await axios.get('http://localhost:9010/briefcase/' + groupId, {
      headers: {
    ...
    group.value = {
      ...
      enableSocialTrading: response.data.enableSocialTrading
    };
